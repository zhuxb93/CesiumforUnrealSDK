...
int splitConvexPolygon(int numvertices,REAL *convexPoly, REAL x1, REAL y1, REAL x2, REAL y2, REAL *polys[]) {
	...
	*/
	...
	int numpolys;
	int i;
	REAL localCompConst = 0.000000000001;
	// for debugging 
	...
	for (i = 0; i < 2*numvertices; i=i+2) {
		...
		if (fabs(p[0]-0.0) <= localCompConst) {
		...
		else if (fabs(p[0]-2.0) <= localCompConst) {
		...
		// ... or if the intersection is a point
			...
			if (fabs(p[1] - convexPoly[j]) <= localCompConst && fabs(p[2] - convexPoly[j+1]) <= localCompConst) {
				// debug
				case31++;
				if (state == 1) {
					poly2counter++;
			...
			else if (!(fabs(p[1] - convexPoly[i]) <= localCompConst && fabs(p[2] - convexPoly[i+1]) <= localCompConst)) {
				// debug
				case32++;
				poly1counter++;
    ...
    REAL x4, REAL y4 , REAL *p)
{
	...
	REAL u_a, u_b, denom;	
	REAL localCompConst = 0.0000000000001;	
	// calculate denominator first
	denom = (y4-y3)*(x2-x1)-(x4-x3)*(y2-y1);
	u_a = (x4-x3)*(y1-y3) - (y4-y3)*(x1-x3);
	u_b = (x2-x1)*(y1-y3) - (y2-y1)*(x1-x3);
	...
	//if(fabs(denom-0.0) < localCompConst && (fabs(u_b-0.0) < localCompConst && fabs(u_a-0.0) < localCompConst)){
 	//printf("denom %.20f u_b  %.20f u_a  %.20f\n",denom, u_b, u_a);
	if(fabs(denom-0.0) < localCompConst){
	 	if(fabs(u_b-0.0) < localCompConst && fabs(u_a-0.0) < localCompConst){
	    ...
	    u_a = u_a/denom;
// 	    printf("u_b %.20f\n", u_b);
	    if( u_b < -localCompConst  || u_b > 1.0 + localCompConst ){	// check if it is on the line segment		
